...
for i in range( documents_to_process ):
    ...
    if(  genres[0]['name'] == 'Drama' or genres[0]['name'] == 'Comendy'  ):
        try:
            if( genres[1]['name'] == 'comedy' or  genres[0]['name'] == 'Drama' ):
                try:
                    feature_set = FeatureSet( i, genres[2]['name'] )
                except:
                    feature_set = FeatureSet( i, genres[1]['name'] )
            else:
                feature_set = FeatureSet( i, genres[1]['name'] )
        except:
    ...
    if feature_set._genre not in unique_genres.keys():
        ...
        unique_genres[ feature_set._genre ] = unique_genres[ feature_set._genre ] + 1
...
for word in vocabulary.keys():
    ...
    for g in unique_genre_probabilities.keys():
        for i in vocabulary[ word ]:
            ...
            # if genre matches current document add all instances of word apperance
            if( feature_sets[i]._genre == g ):
